...
def eval_func(func, n): #helper function
    func_name = func.__name__
    if func_name not in dict_funcs:
        dict_funcs[func_name] = {} #if dont have that name in the dictionary yet add it
    dict_func = dict_funcs[func_name]
    if n not in dict_func:
        dict_func[n] = func(func, n) #only compute results we didnt calculate before
    return dict_func[n]
...
def f13_exam1_fall_2008(func, n):
    if n == 0:
        ...
        return 8*eval_func(func, int(n/4))
...
def call_and_print(func, n, desc):# calls a function and prints an output for it
...
def main():
    ...
    call_and_print(f5_midterm_summer_2021, 256, "f(1) = 2, f(1) = 1, f(n) = 4T(n/3) + n")
    call_and_print(f6_midterm_winter_2022, 256, "f(1) = 1, f(n) = 125f(n/5) + 5n")
    call_and_print(f7_hanoi, 256, "f(1) = 1, f(n) = 2f(n-1) + 1") # bigger than fibonacci
    ...
    call_and_print(f12_sum_cubes, 256, "f(0) = 2, f(1) = 7, f(n) = f(n-1) + 2*f(n-2)")
